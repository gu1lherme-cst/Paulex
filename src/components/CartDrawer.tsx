...
export function CartDrawer() {
  const {
    lines, count, total, isOpen, close, inc, dec, remove, fulfillment, setFulfillment,
  } = useCart();
  const closeRef = useRef<HTMLButtonElement>(null);

  /* Esc fecha, trava o scroll do fundo e gerencia o foco */
  useEffect(() => {
    if (!isOpen) return;
  ...
  const checkout = () => {
    ...
    const itens = lines
      .map(
        (it) =>
          `• ${it.name}\n   ${it.qty} × ${formatBRL(it.priceNum)} = ${formatBRL(it.lineTotal)}`
      )
      .join("\n");
    const msg =
  ...
  return (
    <>
      <div
        className={`px-overlay${isOpen ? " px-overlay--on" : ""}`}
        onClick={close}
        aria-hidden="true"
      />
      <aside
      ...
      >
        ...
        {lines.length === 0 ? (
          <div className="px-drawer__empty">
            ...
            <button type="button" className="px-btn px-btn--primary px-btn--sm" onClick={close}>
              Continuar comprando
            </button>
          </div>
        ) : (
          <>
                ...
                <div className="px-citem" key={it.id}>
                  ...
                  <div className="px-citem__info">
                    ...
                    <div className="px-citem__qty">
                      ...
                      <button type="button" onClick={() => inc(it.id)} aria-label={`Aumentar quantidade de ${it.name}`}>
            ...
            <div className="px-drawer__foot">
              ...
              <button type="button" className="px-drawer__cont" onClick={close}>
                Continuar comprando
              </button>
            </div>
          </>
